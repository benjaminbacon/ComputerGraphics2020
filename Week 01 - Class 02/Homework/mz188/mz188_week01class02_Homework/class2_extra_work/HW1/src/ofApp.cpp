...
void ofApp::setup(){
    ...
    ofSetWindowTitle("color example_Midna version");
    
    ofSetRectMode(OF_RECTMODE_CENTER);
    
    ofSetFrameRate(144); // if vertical sync is off, we can go a bit fast... this caps the framerate at 60fps.
...
void ofApp::draw(){
    ...
    // for floats.
    float hue = fmodf(ofGetElapsedTimef()*50,255);
    ...
    for ( int i=0; i<ofGetWidth(); i+=step )
    {
        ...
        for ( int j=0; j<ofGetHeight(); j+=step )
        {
            ...
            c.setHsb( hue, ofMap(i, 0,ofGetWidth(), 255,255), ofMap(j, ofGetHeight(),0, 255,255 ) );
