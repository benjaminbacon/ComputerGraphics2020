...
void ofApp::setup(){
	...
	ofBackground(ofColor::peachPuff); // let's use a predefined color as our background
	...
	for (int i = 0; i < numBalls; i++) {		// loop 30 times
		...
		SquareBall tempSquareBall;					// create a temporary ball
		squareBalls.push_back(tempSquareBall);		// copy the temporary ball into the balls vector
										// (push_back() adds the ball to the end of the vector, increasing the vector's size by 1)
	}
}
...
void ofApp::update(){
	...
	for (int i = 0; i < squareBalls.size(); i++) {	// one way to loop through a whole vector

		squareBalls[i].update();	// we can access an individual ball by its "index" number in the vector
							// e.g. balls[0] is the first ball in the vector, balls[1] is the second ball, etc.
									// (note that vectors always start at 0, not 1!!)
	}
